...
ShellRoot {
    ...
    // --- Mapeo Catppuccin (referencia) -> Colors.palette[i] (arquitectura del usuario) ---
    // Los indices se infieren del uso real en la config del usuario (paleta luminancia 0=oscuro..9=claro).
    // Patron `Colors.ready ? palette[i] : fallback` para cubrir la ventana de arranque (la paleta carga en ms).
    readonly property color base:     Colors.ready ? Colors.palette[0] : "#1E1E2E"
    readonly property color crust:    Colors.ready ? Colors.palette[0] : "#11111B"
    readonly property color mantle:   Colors.ready ? Colors.palette[1] : "#181825"
    readonly property color text:     Colors.ready ? Colors.palette[7] : "#CDD6F4"
    readonly property color subtext0: Qt.alpha(text, 0.6)
    readonly property color overlay0: Colors.ready ? Colors.palette[6] : "#6C7086"
    readonly property color overlay2: Colors.ready ? Colors.palette[7] : "#9399B2"
    readonly property color surface0: Colors.ready ? Colors.palette[3] : "#313244"
    readonly property color surface1: Colors.ready ? Colors.palette[4] : "#45475A"
    readonly property color surface2: Colors.ready ? Colors.palette[5] : "#585B70"

    readonly property color mauve: Colors.accent   // acento primario (palette[8], fallback interno #89B4FA)
    readonly property color red:   Colors.danger   // fijo #F38BA8
    readonly property color peach: "#FAB387"        // hardcode: no hay rol "warning" en la paleta
    ...
    // Qt.quit() es inerte aqui ("no receivers connected"), asi que sin esto el proceso queda
    // vivo como cliente WlSessionLock: se acumulaban instancias y, al desbloquear una, otra
    // seguia reteniendo la pantalla (bucle de re-bloqueo).
    // El delay deja que el compositor procese el unlock antes de morir; el flock del listener
    // garantiza instancia unica, por lo que el patron solo puede matchear a este proceso.
    Timer {
        id: exitTimer
        interval: 300
    ...
    WlSessionLock {
        ...
        WlSessionLockSurface {
            ...
            Item {
                ...
                // --- Responsive Scaling Logic (Scaler/WindowRegistry.getScale replicado inline) ---
                // min(w/1920, h/1080); piso 0.35 con pow(r,0.85) al reducir, pow(r,0.5) al ampliar.
                // userScale fijo en 1.0 (se descarta la lectura de settings.json de la referencia).
                readonly property real sc: {
                    let w = screenRoot.width  > 0 ? screenRoot.width  : (surface.width  > 0 ? surface.width  : 1920);
                    let h = screenRoot.height > 0 ? screenRoot.height : (surface.height > 0 ? surface.height : 1080);
                    if (w <= 0 || h <= 0) return 1.0;
                    let r = Math.min(w / 1920.0, h / 1080.0);
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        Item {
                            ...
                            Layout.preferredWidth: 170 * screenRoot.sc
                            Layout.preferredHeight: 170 * screenRoot.sc
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                Rectangle {
                                    Layout.preferredWidth: 36 * screenRoot.sc
                            ...
                            Rectangle {
                                id: pinPill
                                Layout.alignment: Qt.AlignLeft
                                Layout.preferredWidth: 280 * screenRoot.sc
                                Layout.preferredHeight: 60 * screenRoot.sc
                                radius: height / 2
                                clip: true
                ...
                RowLayout {
                    ...
                    // Weather Pill (placeholder: sin fuente de datos; cablear a un script propio a futuro)
                    Rectangle {
