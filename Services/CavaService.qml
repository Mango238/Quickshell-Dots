...
Singleton {
    ...
    readonly property string _confPath: `/tmp/dms-cava-${Date.now()}-${Math.floor(Math.random() * 1000000)}.conf`
    Process {
        id: cavaCheck
    ...
    Component.onCompleted: {
        // Higiene: los conf llevan timestamp y se acumulaban uno por sesión
        // en /tmp. Borrar los huérfanos antes de que esta sesión escriba el
        // suyo (cava ya cargó el suyo en sesiones vivas; borrar el archivo
        // no las afecta).
        Quickshell.execDetached(["sh", "-c", "rm -f /tmp/dms-cava-*.conf"]);
