...
Singleton {
    ...
    function connectNodes(srcId, dstId) {
        ...
        if (outs.length === 1 && ins.length > 1) {
            ...
            for (let k = 0; k < outs.length; k++) {
                // Emparejar por canal (FL con FL); si el nodo no declara canales,
                // caer a emparejado por índice.
                const match = ins.find(p => p.ch === outs[k].ch)
                pairs.push([outs[k].id, (match || ins[k % ins.length]).id])
            }
        }

        _mutate(pairs.map(p => `pw-link ${p[0]} ${p[1]}`).join("; "))
    }
    ...
    function disconnectLinks(linkIds) {
        ...
        _mutate(linkIds.map(id => `pw-link -d ${id}`).join("; "))
    }
    ...
    Process {
        ...
        running: false
        command: ["sh", "-c",
            "pw-dump | jq -c '[.[]|select(.type==\"PipeWire:Interface:Port\")"
            + "|{i:.id,n:.info.props[\"node.id\"],d:.info.direction,"
            + "c:(.info.props[\"audio.channel\"]//\"MONO\")}]'"]
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    root.lastError = "pw-dump ilegible: " + e
                    return
                }
                const map = {}
        ...
        onExited: code => {
            if (code !== 0)
                root.lastError = "pw-dump/jq falló (código " + code + ")"
        }
    }
