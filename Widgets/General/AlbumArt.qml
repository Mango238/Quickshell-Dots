...
Item {
    ...
    property bool albumArtStatus: true
    property real albumSize: Math.min(width, height) * 0.88 // 0.88
    property bool showAnimation: true
    property real animationScale: 1.2 // 1.0
    property color accentColor: "#fff"


    readonly property real blobBaseRadiusFactor: 0.43
    readonly property real blobAmplitudeFactor: 0.1 // 0.115
    readonly property real blobOvershoot: 1.15 // 1.15
    readonly property real blobEnergySensitivity: 1.15
    readonly property real cavaFullScale: 45
    readonly property real blobAttack: 0.75 // 0.75
    ...
    readonly property real blobSpinSpeed: 0.05 //0.03

    readonly property bool blobActive: CavaService.cavaAvailable && activePlayer?.playbackState === MprisPlaybackState.Playing && showAnimation
    ...
    FrameAnimation {
        ...
        onTriggered: main.stepBlob(Math.min(frameTime, 0.05))
    }
    ShaderEffect {
        id: blobEffect

        readonly property real span: Math.min(main.width, main.height)

        width: span * (main.blobBaseRadiusFactor + main.blobAmplitudeFactor * main.blobOvershoot) * 2 * main.animationScale + 4
        height: width
        anchors.centerIn: parent
        z: 0
        visible: main.blobActive || activation > 0.004

        property real phase: 0
        property real spin: 0
        property real sizePx: width
        property real baseRadiusPx: span * main.blobBaseRadiusFactor * main.animationScale
        property real amplitudePx: span * main.blobAmplitudeFactor * main.animationScale
        property real activation: main.blobActive ? 1 : 0
